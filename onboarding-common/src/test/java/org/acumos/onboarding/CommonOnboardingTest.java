...
public class CommonOnboardingTest {
	...
	CommonOnboarding on = new CommonOnboarding();
	 ...
	 * 
	 * } catch (IOException e) { assert(true); // TODO Auto-generated catch block
	 * //e.printStackTrace(); }
	 * 
	 * }
	 */
	 ...
	 * localmodelFile.getName()); } File localMetadataFile = new File(outputFolder,
	 * metadata.getOriginalFilename()); try {
	 ...
	 * UtilityFunction.copyFile(schema.getInputStream(), localProtobufFile); } catch
	 * (IOException e) { throw new
	 * AcumosServiceException(AcumosServiceException.ErrorCode.
	 ...
	 * getMetadata()); } finally { UtilityFunction.deleteDirectory(outputFolder); }
	 * } catch (AcumosServiceException e) { HttpStatus httpCode =
	 ...
	 * modelOriginalName = "H20 Model"; String modelId = UtilityFunction.getGUID();
	 * File outputFolder = new File("tmp", modelId); outputFolder.mkdirs(); try {
	 * File localmodelFile = new File(outputFolder,modelOriginalName); try {
	 * UtilityFunction.copyFile(model.getInputStream(), localmodelFile); } catch
	 * (IOException e) { throw new
	 * AcumosServiceException(AcumosServiceException.ErrorCode.
	 * INTERNAL_SERVER_ERROR, "Fail to download model file " +
