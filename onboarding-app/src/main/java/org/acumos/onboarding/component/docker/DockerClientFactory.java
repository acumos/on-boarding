...
package org.acumos.onboarding.component.docker;
			...
			try {
				...
				try {
					...
					throw new IllegalStateException(
							"Unable to craete Instance of DockerCmdExecFactory class: " + cmdExecFactory, e);
				}
			} catch (ClassNotFoundException e) {
				throw new IllegalStateException("Unable to resolve DockerCmdExecFactory class: " + cmdExecFactory, e);
			}
		}
		DockerClient dockerClient = DockerClientBuilder.getInstance(configBuilder).withDockerCmdExecFactory(factory)
				.build();
		return dockerClient;
	}
}
