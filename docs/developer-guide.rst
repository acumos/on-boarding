...
On-boarding allows user to create, or not, containerized microservice at the end of the on-boarding
process for models developped in Java, Python>=3.5, <3.7, R>=3.4.4 and sourced from toolkits such as
Scikit, TensorFlow, H2O, and R. If user choose to not create the microservice during on-boarding he
can create it later.
